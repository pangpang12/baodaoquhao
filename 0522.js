...
/////////////// date 0522 time 1043
app.get('/selected/:_id',function(req,res){
	console.log('床位选择中');
//	var value = {_id:req.params._id};
	pool.getConnection(function(err,connection){
		if (err) throw err;
		var query = connection.query('SELECT Notes FROM Choose WHERE RB=?',{id:req.params._id},function(err,ret){
			if (err) throw err;
			console.log(ret);
			if (ret[0].Notes){
				connection.release();
		...
		console.log(query.sql);
				...
				pool.getConnection(function(err,connection){
					if (err) throw err;
					var query = connection.query('UPDATE Choose SET RN="'+req.session.No+'" WHERE RB=?',{_id:req.params._id},function(err,ret){
						if (err) throw err;
						console.log(ret);
						connection.release();						
					})
				...
				pool.getConnection(function(err,connection){
					if (err) throw err;
					var query = connection.query('UPDATE Student SET Bed=? WHERE Number="'+req.session.No+'"',{_id:req.params._id},function(err,ret){
						if (err) throw err;
						console.log(ret);
						connection.release();
					})
