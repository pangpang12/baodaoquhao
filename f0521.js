...
if(req.session.No){	
	var Gender;
	var locat;
	connection.query('SELECT * FROM Student WHERE Number="'+req.session.No+'"',function(err,ret){
		if (err) throw err;
		console.log(ret);
		Gender = ret[0].Gender;
...
app.get('/Rselect',function(req,res){
	console.log('条件');
console.log('tiaojian xingbie=:'+req.session.Gender);
